...
final class Format
{
    public static function summary($text, $len = 500)
    {
        $text = self::ubbClear($text);
        $text = strip_tags($text);
        $text = self::cutstr($text, $len);
        
        return self::textHtmlFilter($text);
    }
    
    public static function summaryPlainText($text, $len = 500)
    {
        $text = self::ubbClear($text);
        $text = strip_tags($text);
        return self::cutstr($text, $len);
    }
    
    public static function textHtmlFilter($text)
    {   
        $text = self::autoParagraph($text);
        $text = self::ubb2html($text);
    ...
    public static function ubb2html($str, $order="") 
    {
        ...
        $replace = array(
            ...
            "<a href=\"$1\" target=\"_blank\"><img src=\"$1\" border=\"0\" onload=\"javascript:if(this.width>800)this.width=800\" title=\"Open Link in New Window\"></a>",
    ...
    public static function ubbClear($str)
    {
        $match = array(
            "%\[b\](.*?)\[\/b\]%si",
            "%\[center\](.*?)\[\/center\]%si",
