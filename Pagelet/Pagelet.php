...
class LessPHP_Pagelet
{
    ...
    public function renderStatic($ext = NULL, $path = NULL)
    {
        if ($ext === NULL) {
            $ext = array('js', 'css', 'png', 'jpg', 'gif', 'ico');
        }
        $p = parse_url($this->app->uri);
        $t = substr(strrchr($p['path'], "."), 1);
        if (!in_array($t, $ext)) {
            return;
        }
        ...
        echo mime_content_type($file);die();
        ...
        //header("Content-Disposition: attachment; filename='.$p['path']);
